...
netfile = './models/model_joint_10_operator.net'
savepath = './results/'
...
for m in range(len(type)):
    ...
    loss_mse_ave = 0
    loss_psnr_ave = 0
    print('Smoothing method is: ' + type[m])
    for n in range(len(para)):
        ...
        for i in range(len(inputNameList)):
            ...
            showName = inputName[7:]
            print('Now is processing the image '+ showName)
            ...
            detail = label - pred
            detail_enhance = pred + 4*detail
            detail_enhance = detail_enhance.numpy()
            detail_enhance = np.clip(detail_enhance, 0, 255)
            detail_enhance = detail_enhance.transpose(1, 2, 0).astype("uint8")
            save_name = './results/'+ inputName[7:-4]+'_DL4.png'
            cv2.imwrite(save_name, detail_enhance)
            print('............')